...
export async function POST(req: NextRequest) {
  ...
  const { title, author, details, slug, password, masterPassword } = body;
  ...
  await sql`
    INSERT INTO projects (id, slug, title, author, details, password_hash, password)
    VALUES (${id}, ${cleanSlug}, ${title}, ${author || null}, ${details || null}, ${hash}, ${password})
  `;

  return NextResponse.json({ id, slug: cleanSlug, title });
}
