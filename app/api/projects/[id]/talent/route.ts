...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const {
    tab_id, name, age, agency, agent, agent_contact, manager, manager_contact,
    deal_status, availability, notes, imdb_id, photo_url, status,
  } = body;
  if (!name) return NextResponse.json({ error: 'Name required' }, { status: 400 });

  // Duplicate check — case-insensitive, whole project
  const { rows: dupes } = await sql`
    SELECT id FROM talent
    WHERE project_id = ${id} AND LOWER(TRIM(name)) = LOWER(TRIM(${name}))
    LIMIT 1
  `;
  if (dupes.length > 0) {
    return NextResponse.json({ error: `${name} is already in this project` }, { status: 409 });
  }

  // Sort order: min-1 so new talent appears at top
  const { rows: minRows } = await sql`
    SELECT COALESCE(MIN(sort_order), 1) AS min FROM talent
    WHERE project_id = ${id} AND (tab_id = ${tab_id || null} OR (tab_id IS NULL AND ${tab_id || null}::text IS NULL))
  `;
  const nextOrder = (minRows[0]?.min ?? 1) - 1;
  ...
  const { rows } = await sql`
    ...
    VALUES (
      ...
      ${manager || null}, ${manager_contact || null},
